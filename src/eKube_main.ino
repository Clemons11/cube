...
void SendData() {
  ...
  {
    SPI.transfer(start_frame);
    Serial.println(start_frame, HEX);
  }
  delayMicroseconds(1);
  for (i = 0; i < NUM_LEDS; i++)
  {
    SPI.transfer(led_frame[i][0]);
    Serial.println(led_frame[i][0], HEX);
    delayMicroseconds(1);
    SPI.transfer(led_frame[i][1]);
    Serial.println(led_frame[i][1], HEX);
    delayMicroseconds(1);
    SPI.transfer(led_frame[i][2]);
    Serial.println(led_frame[i][2], HEX);
    delayMicroseconds(1);
    SPI.transfer(led_frame[i][3]);
    Serial.println(led_frame[i][3], HEX);
    delayMicroseconds(1);
  }
